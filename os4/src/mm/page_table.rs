...
pub fn translated_ptr<T>(token: usize, ptr: *const T) -> *const T {
    let page_table = PageTable::from_token(token);

    let start_va = VirtAddr::from(ptr as usize);
    let ppn = page_table.translate(start_va.floor()).unwrap().ppn();

    (PhysAddr::from(ppn).0 + start_va.page_offset()) as *const _
}

pub fn translated_mut_ptr<T>(token: usize, ptr: *mut T) -> *mut T {
    let page_table = PageTable::from_token(token);

    let start_va = VirtAddr::from(ptr as usize);
    let ppn = page_table.translate(start_va.floor()).unwrap().ppn();

    (PhysAddr::from(ppn).0 + start_va.page_offset()) as *mut _
}